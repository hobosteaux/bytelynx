...
class Shortlist():
	...
	def _try_add(self, contact):
		...
		if not (self.search_space.Any(lambda x: x.hash_ == contact.hash_)):
			...
			# if == K contacts, and < K contacted.
				...
				max = self.find_min()
				if (contact.hash_ ^ self.target_hash < max.contact.hash_ ^ self.target_hash):
	...
	def get_next(self):
		...
		if (self.searched <= K) and (self.search_space.any(lambda x: not x.contact)):
			item = self.find_min()
			item.contacted = True
			self.searched += 1
			self.in_progress.append(InProgress(item.contact.Address, datetime.now()))
			return item.contact
		else:
			return None
	...
	def find_min(self, uncontacted=True):
		...
		for contact in self.search_space:
			if (uncontacted and not contact.contacted):
				if (contact.contact.hash_ ^ self.target_hash < minCon.contact.hash_ ^ self.target_hash):
					minCon = contact
	...
	def find_max(self, uncontacted=True):
		...
		for contact in reversed(self.search_space):
			if (uncontacted and not contact.contacted):
				if (contact.contact.hash_ ^ self.target_hash > maxCon.contact.hash_ ^ self.target_hash):
					maxCon = contact
			elif (not uncontacted):
				if (contact.contact.hash_ ^ self.target_hash > maxCon.contact.hash_ ^ self.target_hash):
					maxCon = contact
	...
	def __str__(self):
		return '%s : %s - P:%d F:%d T:%d' % (self.target_hash, self.closest, len(self.in_progress), self.searched, K)
			...
			while (len(shortlist.in_progress) < A):
				...
				if (next_min == None):
					...
					if (len(shortlist.in_progress) == 0):
						shortlist.on_full_or_found(shortlist.target_hash, shortlist.closest)
					break
				else:
...
if (__name__ == '__main__'):
	...
	while(True):
		shortie.start_search(Hash(urandom(20)), [Contact(Address('192.168.0.1', 4000), Hash(b'12345678901234567890'))])
		sleep(1)
